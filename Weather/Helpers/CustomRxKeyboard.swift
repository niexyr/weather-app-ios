...
public class RxKeyboard: NSObject, RxKeyboardType {
    ...
    public let curve = BehaviorSubject<UIView.AnimationCurve?>(value: nil)
    public let duration = BehaviorSubject<TimeInterval?>(value: nil)
            ...
            .do(
                onNext: { [weak self] notification in
                    if let curveValue = notification.userInfo?[UIResponder.keyboardAnimationCurveUserInfoKey] as? NSNumber {
                        self?.curve.onNext(UIView.AnimationCurve(rawValue: curveValue.intValue))
                    }
                    
                    if let durationValue = notification.userInfo?[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double {
                        self?.duration.onNext(durationValue)
                    }
                }
            )
